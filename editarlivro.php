...
$id = $_GET['id'] ?? null;
if (!$id || !is_numeric($id)) {
    header("Location: listarlivro.php?erro=ID inválido");
    exit;
}
...
if (!$livro) {
    header("Location: listarlivro.php?erro=Cadastro não encontrado");
    exit;
}
    ...
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 min-w-0">
            ...
            <header class="sticky top-0 bg-cream/80 backdrop-blur-md z-10 border-b border-gray-300 bg-opacity-90" style="background-color: rgba(245, 240, 229, 0.9);">
                <div class="p-4">
                    <h1 class="text-xl font-bold">Editar Cadastro de Alunos</h1>
                </div>
                
 <form id="cadastroForm" action="atualizarLIVRO.php" method="POST">
                ...
                <div class="form-grid">
                    <table style="width:50%" align="center" >
						<tr > 
							...
							<td> <label for="codigo" class="block text-xs font-medium text-gray-700">Código</label>		</td>
							<td> <input type="text" id="codigo" name="codigo"value="<?= htmlspecialchars($livro['codigo'] ??'') ?>" class="w-full border border-gray-300 rounded-md focus:outline-none">		</td>
						</tr>	 <input type="hidden" id="id" name="id" value="<?= htmlspecialchars($livro['id']) ?>"> 
																
							
															
						<tr >	<td> <label for="titulo" class="block text-xs font-medium text-gray-700">Título</label>	</td>
							<td> <input type="text" id="titulo" name="titulo" value="<?= htmlspecialchars($livro['titulo'] ??'') ?>"class="w-full border border-gray-300 rounded-md focus:outline-none"> </td>
						</tr >	
						<tr >	<td> <label for="autor" class="block text-xs font-medium text-gray-700">Autor</label></td>
							<td> <input type="text" id="autor" name="autor" value="<?= htmlspecialchars($livro['autor'] ??'') ?>" class="w-full border border-gray-300 rounded-md focus:outline-none"> </td>
						</tr >
						<tr >	<td> <label for="editora" class="block text-xs font-medium text-gray-700">Editora</label>	</td>
						<td> <input type="text" id="editora" name="editora" value="<?= htmlspecialchars($livro['editora'] ??'') ?>" class="w-full border border-gray-300 rounded-md focus:outline-none"> </td>
						</tr >
						<tr >		<td> <label for="data_cadastro" class="block text-xs font-medium text-gray-700">Data de Cadastro</label> </td>
				
								<td> <input type="date" id="data_cadastro" name="data_cadastro" value="<?= htmlspecialchars($livro['data_cadastro'] ??'') ?>"class="w-full border border-gray-300 rounded-md focus:outline-none" >		</td>
							</tr>	                    		
						</tr >				
