...
$id = $_GET['id'] ?? null;
if (!$id || !is_numeric($id)) {
    header("Location: listaraluno.php?erro=ID inválido");
    exit;
}
...
if (!$aluno) {
    header("Location: listaraluno.php?erro=Cadastro não encontrado");
    exit;
}
...
<body>
 ...
 <form id="cadastroForm" action="atualizaraluno.php" method="POST">
                ...
                <div class="form-grid">
                    <table style="width:50%" align="center" >
						<tr > 
							...
							<td> <input type="text" id="nome" name="nome"value="<?= htmlspecialchars($aluno['nome']) ?>" class="w-full border border-gray-300 rounded-md focus:outline-none">		</td>
						</tr>	 <input type="hidden" id="id" name="id" value="<?= htmlspecialchars($aluno['id']) ?>"> 
						<tr >	<td> <label for="serie" class="block text-xs font-medium text-gray-700">Serie</label>	</td> 
								<td> <select id="serie" name="serie" value="<?= htmlspecialchars($aluno['serie']) ?>"class="w-full border border-gray-300 rounded-md focus:outline-none">		
						...
						<tr >	<td> <label for="turma" class="block text-xs font-medium text-gray-700">Turma</label>	</td>
							<td> <input type="text" id="turma" name="turma" value="<?= htmlspecialchars($aluno['turma']) ?>"class="w-full border border-gray-300 rounded-md focus:outline-none"> </td>
						</tr >			
						<tr >		<td> <label for="data_cadastro" class="block text-xs font-medium text-gray-700">Data de Cadastro</label> </td>
				
								<td> <input type="date" id="data_cadastro" name="data_cadastro" value="<?= htmlspecialchars($aluno['data_cadastro']) ?>"class="w-full border border-gray-300 rounded-md focus:outline-none" >		</td>
							</tr>	                    		
						</tr >				
